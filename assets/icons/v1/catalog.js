...
(function (root) {
 'use strict';
 const items = [
{"id": "charcoal", "name": "木炭", "group": "goods", "file": "charcoal", "cue": "深炭紫灰；两块错位短炭，双瓣不规则轮廓", "use": "扩展生产链"},{"id": "paper", "name": "纸张", "group": "goods", "file": "paper", "cue": "暖象牙白；单张竖向纸页，右上大卷角", "use": "扩展生产链"},{"id": "book", "name": "书籍", "group": "goods", "file": "book", "cue": "靛蓝；厚书竖向圆角矩形，弧形书脊", "use": "扩展生产链"},{"id": "machine", "name": "机械", "group": "goods", "file": "machine", "cue": "赤铜；六个圆钝齿的粗厚环形齿轮", "use": "扩展生产链"},{"id": "kiln", "name": "炭窑", "group": "building", "file": "kiln", "cue": "圆窑、烟囱与炭堆", "use": "扩展生产链"},{"id": "paperMill", "name": "造纸坊", "group": "building", "file": "paperMill", "cue": "晾纸架与浅色纸堆", "use": "扩展生产链"},{"id": "printer", "name": "印刷坊", "group": "building", "file": "printer", "cue": "压印机与蓝色书本", "use": "扩展生产链"},{"id": "machineWorks", "name": "机械厂", "group": "building", "file": "machineWorks", "cue": "齿轮、厂房与高烟囱", "use": "扩展生产链"},
  ...
  {id:'log',name:'原木',group:'goods',file:'log',cue:'陶土棕；短粗圆柱，左前方大圆端',use:'三条加工链的共同输入'},
  {id:'board',name:'木板',group:'goods',file:'board',cue:'蜂蜜赭黄；两块错位宽板，圆钝长方形',use:'与圆截面的原木区分'},
  {id:'stone',name:'石头',group:'goods',file:'stone',cue:'暖灰褐；单块宽底圆顶巨石',use:'与尖锐的铁矿石区分'},
  {id:'tool',name:'石头工具',group:'goods',file:'tool',cue:'鼠尾草灰绿 + 棕柄；大横锤头与粗短柄的 T 形',use:'石匠铺产物，不能用于拆除按钮'},
  {id:'ore',name:'铁矿石',group:'goods',file:'ore',cue:'铁锈红褐；高低双峰矿块，上窄下宽',use:'原料；不能直接出售给城镇'},
  {id:'iron',name:'铁',group:'goods',file:'iron',cue:'冷银蓝；单枚宽梯形锭，顶窄底宽',use:'炼制成品，与天然矿石区分'},
 ...
 function artwork(id,base){
  ...
  return `<img src="${escape(base+meta.file+'?v=rounded5')}" alt="" draggable="false" style="width:${meta.width*s*100}%;height:${meta.height*s*100}%;left:${(.5-(b.x+b.width/2)*s)*100}%;top:${(.5-(b.y+b.height/2)*s)*100}%">`;
 }
 ...
 function svgIcon(id,{base='assets/icons/v1/',x=0,y=0,size=48}={}){
  ...
  return `<g class="tt-map-icon" aria-hidden="true" pointer-events="none" transform="translate(${Number(x)} ${Number(y)}) scale(${Number(size)/64})"><image href="${escape(base+meta.file+'?v=rounded5')}" x="${32-(b.x+b.width/2)*s}" y="${32-(b.y+b.height/2)*s}" width="${meta.width*s}" height="${meta.height*s}"/></g>`;
 }
