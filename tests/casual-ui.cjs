...
(async()=>{const browser=await chromium.launch({channel:'msedge',headless:true});try{
 for(const width of [1440,390]){
  ...
  let text=await page.locator('#selection').innerText();assert.match(text,/固定产量/);assert.match(text,/11 件木板/);assert.match(text,/已接通/);assert.doesNotMatch(text,/自动效率|积压|满仓|瓶颈|运入|消耗/);
  ...
  await page.locator('#selection [data-command]').filter({hasText:'升级工艺'}).click();assert.equal(await page.evaluate(()=>world.tech.sawmillCraft),before.craft+1);
  await page.locator('#selection summary').click();assert.ok(await page.locator('#connect-accessible').isVisible());await page.locator('#connect-accessible').click();assert.equal(await page.evaluate(()=>connectFrom),'0,0');await page.keyboard.press('Escape');
  ...
  assert.equal(await page.evaluate(()=>townRevenue(E.START_TOWN,'log')),old);assert.match(await page.locator('#selection').innerText(),/售价加成 25%/);
  ...
  const saved=await page.evaluate(()=>JSON.stringify(localStorage));await page.goto(pathToFileURL(path.resolve('docs/ui-components.html')).href);assert.equal(await page.locator('.workshop-metrics').count(),1);assert.equal(await page.evaluate(()=>JSON.stringify(localStorage)),saved);
