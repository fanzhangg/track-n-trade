...
test('a passive policy opens all eight industries and keeps buying without a long saving wall',()=>{
 for(const seed of [1,2,42]){
  const r=M.simulate({seed,rounds:600,maxFlowers:16,clicks:0});
  for(const good of ['log','stone','board','tool','iron','paper','book','machine'])assert.ok(r.firstSales[good]>0&&r.firstSales[good]<400,`${seed}: ${good} opportunity became a sale`);
