...
function world(){const w=E.newWorld(7);w.money=1e8;E.command(w,{type:'build',tile:E.START_TILE,buildType:'camp'});E.command(w,{type:'connect',from:E.START_TILE,to:E.START_TOWN});return w;}
function crew(w,tile,n){for(let i=0;i<n;i++)E.command(w,{type:'worker',tile});}
test('output yard limits surplus, not 30 pieces per round delivered along a road',()=>{
 ...
 assert.equal(w.stats.reduce((n,s)=>n+s.sales[E.START_TOWN].log,0)/E.WINDOW,30);
 ...
 assert.equal(w.tiles[E.START_TILE].loose.log,20);assert.equal(w.stats.at(-1).tiles[E.START_TILE]||0,0);
});
test('processing input batches and output dispatch both exceed 20 without losing goods',()=>{
 ...
 run(w,240);assert.equal(w.stats.reduce((n,s)=>n+s.sales[E.START_TOWN].board,0)/E.WINDOW,30);
 for(const tile of [w.tiles[E.START_TILE],t])assert.ok(tile.loose[E.RECIPES[tile.building.type].out]<=20);
});
...
test('after introducing all goods, new markets combine two goods rather than reverting to repeated log towns',()=>{
 ...
 for(let n=1;n<=14;n++){
  ...
  if(n>8&&fog.design.buys){assert.equal(Object.keys(fog.design.buys).length,2);paired++;}
 }
 assert.ok(paired>=3);E.validate(w);
});
test('a two-input factory can deliver 30 per round without material loss or a 20-piece throughput cap',()=>{
 const w=world();for(const key of ['quarry','sawmill','mason'])E.command(w,{type:'tech',key});
 ...
 assert.equal(w.stats.reduce((n,s)=>n+s.sales[E.START_TOWN].tool,0)/E.WINDOW,30);
...
test('an idle nearby producer does not cap shipments from a distant working producer',()=>{
 ...
 run(w,240);
 assert.equal(w.stats.reduce((sum,s)=>sum+s.sales[town.id].log,0)/E.WINDOW,30);
