...
document.getElementById('goods-list').innerHTML=[['log','原木','◉'],['board','木板','▤'],['stone','石头','◆'],['tool','石头工具','⚒'],['ore','铁矿石','▲'],['iron','铁','▰'],['charcoal','木炭',''],['paper','纸张',''],['book','书籍',''],['machine','机械','']].map(([token,name,icon])=>`<div class="good">${TradeIcons.icon(token,{base:'../assets/icons/v1/',size:24,decorative:true})}<span>${name}<code>${value('goods-'+token).toUpperCase()}</code></span></div>`).join('');
